import {popUp, addEmptyState, getTimeDef} from "./functions.js";
...
$(document).ready(function() {
    ...
    createTask.click(function () {
        ...
        $(`#popWindowTask input[type="submit"]`).click( function (event) {

            if ($(`#popWindowTask input[type="submit"]`).val() === "Create") {
                console.log("create is pressed")

                event.preventDefault()

                sessionStorage.setItem("taskTitle", $(`#popWindowTask input[type="text"]`).val())
                sessionStorage.setItem("taskProject", $(`#popWindowTask select`).val())
                sessionStorage.setItem("taskTime", $(`#popWindowTask input[type="time"]`).val())
                sessionStorage.setItem("taskDate", $(`#popWindowTask input[type="date"]`).val())

                let taskBlock = `<li class="task" data-popup-name="popWindowTask">
    <div class="checkbox"><input type="checkbox"></div>
    <div class="mainData"><h4 class="title">${sessionStorage.getItem("taskTitle")}</h4>
    <div class="deadline">
              ...
              <p>${getTimeDef($(`#popWindowTask input[type="date"]`).val())} till ${sessionStorage.getItem("taskTime")}</p></div>
    </div>
    <div class="secondaryData">
    <div class="user">
        <img src="../imgs/nav/Avatar.png" width="20" alt="avatar">
</div>
    <div class="project">
        <h6 class="projectName">${sessionStorage.getItem("taskProject")}</h6>
        <span class="projectChar">${sessionStorage.getItem("taskProject")[0].toUpperCase()}</span>
    </div>
</div>
</li>`

                $("#tasks .tasks__list").append(taskBlock)

                $("#tasks .empty__state").css("display", "none")
                $("#tasks .tasks__list").css("display", "flex")
            }

            $(`#popWindowTask input[type="submit"]`).off("click")
        })
    })
    ...
    createProject.click(function () {
        ...
        $("#popWindowProject input[type='submit']").click(function (event) {
            event.preventDefault()
          ...
          <div class="project__info">
            ...
            $("#projects .projects").append(projectBlock) // добавляем project элемент

            $("#popWindowProject input[type='submit']").off("click")
        })
    ...
    createWGoal.click(function () {
        ...
        $("#popWindowWeekGoal textarea").val("")
